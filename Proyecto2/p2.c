...
int main(void) {
    ...
    pthread_create(&threadSend, NULL, &sendMsg, msqid2);
    pthread_create(&threadRecv, NULL, &recvMsg, msqid1);
...
void *sendMsg(void *msqid){
    ...
    printf("message queue [2]: done sending messages.\n");
}
...
void *recvMsg(void *msqid){
    ...
    printf("message queue 2: done receiving messages.\n");
    system("rm msgq2.txt");
}